// Package toolsmitherr is the structured error type every verb raises for a
...
// separate code paths that could diverge (CONTRACT.md C2.5).
package toolsmitherr
...
// Render writes err to w in the shape the contract fixes: one
// "toolsmith: <verb>: <message>" line in human mode, or the {"error": {"code",
...
func Render(w io.Writer, verb string, err *Error, jsonMode bool) {
	...
	if verb == "" {
		_, _ = fmt.Fprintf(w, "toolsmith: %s\n", err.Message)
		return
	}
	_, _ = fmt.Fprintf(w, "toolsmith: %s: %s\n", verb, err.Message)
}
