...
// manifestDigest computes the manifest_digest field (C3.4): sha256 over m's
// own canonical JSON encoding with manifest_digest itself held empty,
// hex-encoded with a "sha256:" prefix — the same digest shape Checksum uses
// for one asset file, applied to the whole document. Build calls this only
