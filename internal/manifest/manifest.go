...
// (CONTRACT.md C3): the flat JSON object `toolsmith manifest --json` emits —
...
// names, the same package path — so `toolsmith version` and
// `toolsmith manifest --json`'s tool object are never two sources for one
// fact.
type Tool struct {
...
// Manifest is the flat object `toolsmith manifest --json` emits.
