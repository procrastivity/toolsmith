// Package install implements the `toolsmith install [harness]` verb: it
...
// Command constructs the `toolsmith install <harness>` verb. root is the
...
func Command(streams *iostreams.Streams, build buildinfo.Info, root *cobra.Command) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install <harness>",
		Short: "render and install toolsmith's self-projection into an agent harness",
		Long: "render and install toolsmith's self-projection into an agent harness.\n\n" +
			"With no harness name, toolsmith install detects every harness available on this host and installs into each one, reporting a result per harness. Naming a harness installs (or reinstalls) only that one.\n\n" +
	...
	cmd.Flags().BoolVar(&force, "force", false, "overwrite a target tree even if it was hand-edited or not written by toolsmith install")
...
// harnessResult exactly as toolsmitherr.Error would render them under
// --json.
...
// installAll is the bare `toolsmith install` (no harness argument) path: it
...
func writeInstallAllResults(streams *iostreams.Streams, flags cliflags.Flags, results []harnessResult) error {
	...
	if !anyDetected {
		if _, err := fmt.Fprintln(streams.Out, "no harness detected on this host; install one explicitly: toolsmith install <harness>"); err != nil {
			return err
		}
	}
	return nil
