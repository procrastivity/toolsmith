...
func Risk(harnessName, dir string, s State) string {
	switch s {
	case UnownedConflict:
		return fmt.Sprintf("%s holds content with no install stamp; it was not written by `toolsmith install %s`", dir, harnessName)
	case Modified:
		return fmt.Sprintf("%s no longer matches what `toolsmith install %s` last wrote", dir, harnessName)
	case Incompatible:
		return fmt.Sprintf("%s carries a stamp `toolsmith install %s` cannot use (unparseable, or from a schema version this binary does not recognize)", dir, harnessName)
	default:
		return ""
	}
}
...
func Refusal(harnessName, dir string, s State, remedy string) error {
	...
	return toolsmitherr.New(code, fmt.Sprintf("refused — %s — %s", Risk(harnessName, dir, s), remedy))
}
