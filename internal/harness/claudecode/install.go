...
func Uninstall() (string, error) {
	...
	if !ok {
		return "", toolsmitherr.New("refusal.unstamped-harness-target",
			fmt.Sprintf("refused — %s has no install stamp; it was not written by `toolsmith install %s` and will not be removed automatically", dir, Name))
	}
	...
	if drift := manifest.Drift(actual, stamp); len(drift) > 0 {
		return "", toolsmitherr.New("refusal.unstamped-harness-target",
			fmt.Sprintf("refused — %s no longer matches what `toolsmith install %s` last wrote (%d file(s) changed since); remove it by hand if that was intentional", dir, Name, len(drift)))
	}
