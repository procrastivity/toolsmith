...
func Execute(root *cobra.Command, streams *iostreams.Streams) int {
	...
	if errors.As(err, &terr) {
		...
		toolnameerr.Render(streams.Err, verbPath(root, cmd), terr, jsonOut)
	...
	_, _ = fmt.Fprintf(streams.Err, "toolname: %s\n", err)
...
func classifyRunEFailures(root *cobra.Command) {
	...
	root.RunE = func(cmd *cobra.Command, args []string) error {
		...
		var structured *toolnameerr.Error
		var silent *exitcode.SilentError
		if errors.As(err, &structured) || errors.As(err, &silent) {
			return err
		}
		return toolnameerr.New("internal.command-failed", err.Error())
	}
}

// verbPath names the failing verb for the human-mode
// "toolname: <verb>: <message>" line — the full subcommand path minus the
