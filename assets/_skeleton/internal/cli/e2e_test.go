...
func TestUsageError_UnknownCommand(t *testing.T) {
	...
	if r.exitCode != 2 || r.stdout != "" || !strings.HasPrefix(r.stderr, "toolname: ") {
...
func TestInstall_RunEFailureIsInternalAndArgsFailureRemainsUsage(t *testing.T) {
	...
	env := []string{"TOOLNAME_CLAUDE_SKILLS_DIR=" + skillsFile}
	text := run(t, env, "install", "claude-code", "--force")
	if text.exitCode != 4 || text.stdout != "" || !strings.HasPrefix(text.stderr, "toolname: install: claudecode: creating ") || !strings.Contains(text.stderr, "not a directory") {
...
func TestInstall_MalformedDescriptionOverride(t *testing.T) {
	for name, content := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			xdg := t.TempDir()
			override := filepath.Join(xdg, "toolname", "templates", "skills", "claude-code", "description.txt")
			...
			env := []string{"TOOLNAME_CLAUDE_SKILLS_DIR=" + t.TempDir(), "XDG_CONFIG_HOME=" + xdg}
