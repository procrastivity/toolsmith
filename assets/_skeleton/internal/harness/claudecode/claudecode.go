...
func resolveSkillDescription() (string, error) {
	...
	if desc == "" || strings.ContainsAny(desc, "\n\r") {
		return "", toolnameerr.New("validation.skill-description",
			fmt.Sprintf("skill description asset %q must be exactly one non-empty line; fix or remove the override under the toolname config directory", skillDescriptionAsset))
	}
	return desc, nil
}
