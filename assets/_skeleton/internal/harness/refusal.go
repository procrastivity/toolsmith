...
func Refusal(harnessName, dir string, s State, remedy string) error {
	...
	case UnownedConflict:
		return toolnameerr.New(CodeUnownedConflict,
			fmt.Sprintf("refused — %s holds content with no install stamp; it was not written by `toolname install %s` — %s", dir, harnessName, remedy))
	case Modified:
		return toolnameerr.New(CodeModified,
			fmt.Sprintf("refused — %s no longer matches what `toolname install %s` last wrote — %s", dir, harnessName, remedy))
	case Incompatible:
		return toolnameerr.New(CodeIncompatible,
			fmt.Sprintf("refused — %s carries a stamp `toolname install %s` cannot use (unparseable, or from a schema version this binary does not recognize) — %s", dir, harnessName, remedy))
	default:
		return nil
	}
}
