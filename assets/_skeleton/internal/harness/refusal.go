...
// Refusal codes, one per refusing state (C4.5, C4.6). One shared code used
// to cover UnownedConflict and Modified, with only the message telling
// them apart; a caller that reads codes can now tell them apart too.
const (
...
func Risk(harnessName, dir string, s State) string {
	switch s {
	case UnownedConflict:
		return fmt.Sprintf("%s holds content with no install stamp; it was not written by `toolname install %s`", dir, harnessName)
	case Modified:
		return fmt.Sprintf("%s no longer matches what `toolname install %s` last wrote", dir, harnessName)
	case Incompatible:
		return fmt.Sprintf("%s carries a stamp `toolname install %s` cannot use (unparseable, or from a schema version this binary does not recognize)", dir, harnessName)
	default:
		return ""
	}
}
...
func Refusal(harnessName, dir string, s State, remedy string) error {
	...
	return toolnameerr.New(code, fmt.Sprintf("refused — %s — %s", Risk(harnessName, dir, s), remedy))
}
