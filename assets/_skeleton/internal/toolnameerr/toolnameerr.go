// Package toolnameerr is the structured error type every verb raises for a
...
// "toolname: <verb>: <message>" line in human mode, or the {"error": {"code",
...
func Render(w io.Writer, verb string, err *Error, jsonMode bool) {
	...
	if verb == "" {
		_, _ = fmt.Fprintf(w, "toolname: %s\n", err.Message)
		return
	}
	_, _ = fmt.Fprintf(w, "toolname: %s: %s\n", verb, err.Message)
}
