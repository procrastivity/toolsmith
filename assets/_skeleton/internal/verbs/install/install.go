// Package install implements the `toolname install [harness]` verb: it
...
func Command(streams *iostreams.Streams, build buildinfo.Info, root *cobra.Command) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install <harness>",
		Short: "render and install toolname's self-projection into an agent harness",
		Long: "render and install toolname's self-projection into an agent harness.\n\n" +
			"With no harness name, toolname install detects every harness available on this host and installs into each one, reporting a result per harness. Naming a harness installs (or reinstalls) only that one.\n\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !slices.Contains(registry.Names, harnessName) {
				...
				return toolnameerr.New("validation.unknown-harness",
	...
	cmd.Flags().BoolVar(&force, "force", false, "overwrite a target tree even if it was hand-edited or not written by toolname install")
...
func forceRemedy(harnessName string, s harness.State) string {
	...
	return fmt.Sprintf("re-run with `toolname install %s --force`, %s", harnessName, consequence)
}
...
// harnessResult exactly as toolnameerr.Error would render them under
// --json.
...
// installAll is the bare `toolname install` (no harness argument) path: it
...
func installAll(streams *iostreams.Streams, flags cliflags.Flags, m manifest.Manifest, force bool) error {
	...
	if len(refused) > 0 {
		return toolnameerr.New("refusal.harness-targets-refused",
			fmt.Sprintf("refused — %d harness target(s) hold hand-edited, unstamped, or incompatible content (%s); re-run `toolname install <harness> --force` for each to overwrite",
...
func writeInstallAllResults(streams *iostreams.Streams, flags cliflags.Flags, results []harnessResult) error {
	...
	if !anyDetected {
		if _, err := fmt.Fprintln(streams.Out, "no harness detected on this host; install one explicitly: toolname install <harness>"); err != nil {
			return err
		}
	}
	return nil
