...
// tree on disk matches it too (C4.6's both-sides comparison). All three
// must hold — a missing stamp, a binary that would generate something
// different (a verb added since), or a file changed on disk all make the
// tree not current. It is the install verb's "nothing to write" test, run
// after RefuseHandEdited has cleared the tree; doctor's stale-artifact
// check asks the middle question alone (binary vs stamp) and reports
// drift, where this asks whether a rewrite would change anything at all.
func IsCurrent(dir string, files map[string][]byte) (bool, error) {
	...
	// (C4.6) — only the checksums below decide.
